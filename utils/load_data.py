...
the adjacency matrix. The code is mostly taken from the original repo with minor adjustments
'''
import numpy as np
import random as rd
...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        t1 = time()
        self.R_train = sp.dok_matrix((self.n_users, self.n_items), dtype=np.float32) #  dok 维数为2 ：S[i, j] = i + j
        self.R_test = sp.dok_matrix((self.n_users, self.n_items), dtype=np.float32)

        # create two dicts("train/test_items") to store [user,id]
    ...
    def create_adj_mat(self):
        ...
        adj_mat = sp.dok_matrix((self.n_users + self.n_items, self.n_users + self.n_items), dtype=np.float32) # mm矩阵（m = 所有用户+item）
        ...
        def normalized_adj_single(adj):
            ...
            norm_adj = d_mat_inv.dot(adj).dot(d_mat_inv) # dot: 矩阵乘法
